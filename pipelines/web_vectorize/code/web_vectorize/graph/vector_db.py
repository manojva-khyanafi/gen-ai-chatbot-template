...
def vector_db(spark: SparkSession, web_silver_content_vectorized_1: DataFrame):
    ...
    if spark.catalog.tableExists("prophecy_data.operational.pinecone_vectors_upsert_status"):
        web_silver_content_vectorized_1\
            ...
            .withColumn("upserted", expr("pinecone_upsert(\"all-vectors\", id_vectors)"))\
            .select(col("*"), col("upserted.*"))\
            .select(col("id_vectors"), col("count"), col("error"))\
            .write\
            .format("delta")\
            .insertInto("prophecy_data.operational.pinecone_vectors_upsert_status")
    else:
        web_silver_content_vectorized_1\
            .withColumn("_row_num", row_number().over(Window.partitionBy().orderBy(col("id"))))\
            .withColumn("_group_num", ceil(col("_row_num") / 20))\
            ...
            .format("delta")\
            .mode("overwrite")\
            .saveAsTable("prophecy_data.operational.pinecone_vectors_upsert_status")
